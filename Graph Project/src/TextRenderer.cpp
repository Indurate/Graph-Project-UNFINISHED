...
TextRenderer::TextRenderer(const char* const file, int fontSize) : fileName(file) {
	loadFile(file, fontSize);
	loadGlyphs();
}
...
int TextRenderer::getLengthOfString(std::string& text, float scale) {
	float x = 0;
...
{
	x += ScreenData::getWidth() / 2;
	y += ScreenData::getHeight() / 2;
	getLengthOfString(text, scale);
	s.bind();
	glEnable(GL_CULL_FACE);
	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	glm::mat4 projection = glm::ortho(0.0f, (float)ScreenData::getWidth(), 0.0f, (float)ScreenData::getHeight());
